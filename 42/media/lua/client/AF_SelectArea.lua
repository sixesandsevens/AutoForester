...
local function toRect(a,b)
  ...
  local x1 = math.min(a[1], b[1]); local x2 = math.max(a[1], b[1])
  local y1 = math.min(a[2], b[2]); local y2 = math.max(a[2], b[2])
  return {x1,y1,x2,y2,T.z}
end
local function paintRect(rect)
  clearHi(); rect = AFCore.normalizeRect(rect); if not rect then return end
  ...
  for y=y1,y2 do for x=x1,x2 do
    ...
    if sq and sq.setHighlighted then sq:setHighlighted(true); if sq.setHighlightColor then sq:setHighlightColor(0.2,0.8,1.0) end; table.insert(T.hi, sq) end
  end end
end
...
function AF_SelectArea.onMouseDown(x,y)
  ...
  T.a = { sq:getX(), sq:getY() }; T.b = T.a; paintRect(toRect(T.a,T.b))
end
function AF_SelectArea.onMouseMove(x,y)
  if not T.active or not T.a then return end
  local mx,my = getMouseXScaled(), getMouseYScaled()
  local sq = toSq(mx,my,T.z); if not sq then return end
  T.b = { sq:getX(), sq:getY() }; paintRect(toRect(T.a,T.b))
end
